...
export function ShiftManager({
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-[#141414] p-5">
      ...
      <p className="mt-1 text-xs text-white/45">Starter+: Leitung plant Mitarbeiter, System macht Soll/Ist beim Stempeln.</p>
      {shiftCycleWeeks > 1 && (
        <div className="mt-3 inline-flex rounded-lg border border-white/10 bg-[#0f0f0f] p-1 text-xs">
            ...
            return (
              <button
                ...
                className={`rounded-md px-3 py-1.5 ${
                  selectedWeekIndex === week ? "bg-white/10 text-white" : "text-white/50 hover:text-white/80"
                }`}
              >
                Woche {week}
              </button>
      ...
      <div className="mt-3 inline-flex rounded-lg border border-white/10 bg-[#0f0f0f] p-1 text-xs">
        <button
          ...
          className={`rounded-md px-3 py-1.5 ${viewMode === "timeline" ? "bg-white/10 text-white" : "text-white/50 hover:text-white/80"}`}
        >
          Team-Timeline
        </button>
        <button
          ...
          className={`rounded-md px-3 py-1.5 ${viewMode === "simple" ? "bg-white/10 text-white" : "text-white/50 hover:text-white/80"}`}
        >
          Einfach-Planer
        </button>
        <button
          ...
          className={`rounded-md px-3 py-1.5 ${viewMode === "matrix" ? "bg-white/10 text-white" : "text-white/50 hover:text-white/80"}`}
        >
          Wochenmatrix
        </button>
      </div>
      ...
      <div className="mt-4 grid gap-2 md:grid-cols-5">
        <select
          ...
          className="rounded-lg border border-white/10 bg-[#0b0b0b] px-3 py-2 text-sm"
          disabled={isPending}
        >
          {members.map((m) => (
        ...
        <button
          ...
          className="rounded-lg border border-[#22c55e]/30 bg-[#22c55e]/15 px-3 py-2 text-sm font-semibold text-[#86efac] hover:bg-[#22c55e]/20 disabled:opacity-60"
        >
          Standardwoche (Mo-Fr)
        </button>
        <button
          ...
          disabled={isPending || !selectedUserId}
          className="rounded-lg border border-blue-400/30 bg-blue-500/15 px-3 py-2 text-sm font-semibold text-blue-200 hover:bg-blue-500/20 disabled:opacity-60"
        >
          Auf alle übertragen
        </button>
      </div>
      <div className="mt-2 flex flex-wrap items-center gap-2">
        <p className="text-[11px] text-white/45">Tipp: Zeit oben einstellen und Tage direkt antippen. Erneuter Klick mit gleicher Zeit löscht den Tag.</p>
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-2">
        <div className="rounded-xl border border-cyan-400/20 bg-cyan-500/5 px-3 py-2">
          <p className="text-[10px] uppercase tracking-widest text-cyan-200/80">Schritt 1</p>
          <p className="text-sm text-cyan-100">Zeit oben wählen (oder Früh/Standard/Spät klicken).</p>
        </div>
        <div className="rounded-xl border border-cyan-400/20 bg-cyan-500/5 px-3 py-2">
          <p className="text-[10px] uppercase tracking-widest text-cyan-200/80">Schritt 2</p>
          <p className="text-sm text-cyan-100">Tage antippen. Jeder Klick speichert sofort.</p>
        </div>
      </div>
      {hasInvalidRange && (
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2 text-xs">
        <button
          ...
          className="rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-white/70 hover:bg-white/10"
          disabled={isPending}
        >
          Früh: 08:00-16:00
        </button>
          ...
          <button
            ...
            className={`rounded-lg border px-2 py-2 text-xs transition-colors disabled:opacity-60 ${
              selectedUserVacationDays.has(idx)
                ...
                : usedDays.has(idx)
                ? "border-[#22c55e]/30 bg-[#22c55e]/10 text-[#86efac] hover:bg-[#22c55e]/15"
                : "border-white/10 bg-white/5 text-white/75 hover:bg-white/10"
            } ${recentDayAction?.dayOfWeek === idx ? "ring-2 ring-cyan-300/70" : ""}`}
          >
            <span className="block text-xs">{label}</span>
            <span className="mt-0.5 block text-[10px] font-mono opacity-80">
            ...
            {recentDayAction?.dayOfWeek === idx && (
              <span className={`mt-1 block text-[10px] ${recentDayAction.action === "saved" ? "text-cyan-200" : "text-red-200"}`}>
      ...
      {message && <p className="mt-3 rounded-lg border border-white/10 bg-[#0f0f0f] px-3 py-2 text-xs text-white/75">{message}</p>}

      <div className="mt-4">
        <button
          type="button"
          onClick={() => setShowDetails((v) => !v)}
          className="rounded-md border border-white/15 bg-white/5 px-3 py-1.5 text-xs text-white/75 hover:bg-white/10"
        >
          {showDetails ? "Details ausblenden" : "Details anzeigen"}
        </button>
      </div>
      {showDetails && (
      <div className="mt-3 rounded-xl border border-white/10 bg-[#0f0f0f]">
        <div className="grid grid-cols-3 border-b border-white/10 px-3 py-2 text-[11px] uppercase tracking-widest text-white/35">
          <span>Tag</span>
          <span>Start</span>
          <span>Ende</span>
        </div>
        {userShifts.length === 0 ? (
          <p className="px-3 py-3 text-xs text-white/40">Noch keine Schichten für den ausgewählten Mitarbeiter.</p>
        ) : (
          userShifts.map((s, idx) => (
            <div key={s.id} className={`grid grid-cols-3 items-center px-3 py-2 text-sm ${idx % 2 === 0 ? "bg-white/[0.01]" : ""}`}>
              ...
              <span className="font-mono text-white/70">{s.startTime}</span>
              <span className="font-mono text-white/70">{s.endTime}</span>
            </div>
          ))
        )}
      </div>
      ...
      {viewMode === "matrix" && (
        <div className="mt-4 rounded-xl border border-white/10 bg-[#0f0f0f] overflow-x-auto">
          ...
          <table className="w-full min-w-[760px] text-sm">
            <thead>
              <tr className="border-b border-white/10 text-white/40">
                ...
                {MATRIX_DAYS.map((dayOfWeek) => (
                  <th key={dayOfWeek} className="px-2 py-2 text-center text-xs font-mono uppercase tracking-wider">
            ...
            <tbody>
              {visibleMatrixMembers.map((m) => (
                <tr key={m.id} className="border-b border-white/[0.06] last:border-0">
                  <td className="px-3 py-2 text-white/80">{m.name ?? m.email}</td>
                  {MATRIX_DAYS.map((dayOfWeek) => {
                    const shift = shiftByUserAndDay.get(`${m.id}-${selectedWeekIndex}-${dayOfWeek}`);
                    ...
                    return (
                      <td key={`${m.id}-${dayOfWeek}`} className="px-2 py-2 text-center">
                        <button
                          ...
                          className={`w-full rounded-md border px-2 py-1.5 text-[11px] font-mono transition-colors ${
                            conflictType === "SICK"
              ...
              <tr className="border-t border-white/15 bg-white/[0.02]">
                <td className="px-3 py-2 text-[11px] font-semibold uppercase tracking-widest text-white/55">Stunden-Summe</td>
                {matrixDayTotals.map((minutes, idx) => (
                  <td key={`sum-${MATRIX_DAYS[idx]}`} className="px-2 py-2 text-center text-xs font-semibold text-cyan-200">
                    {formatHours(minutes)}
                  </td>
                ))}
              </tr>
      ...
      {viewMode === "timeline" && (
        <div className="mt-4 rounded-xl border border-white/10 bg-[#0f0f0f] p-3">
          <div className="grid gap-2 md:grid-cols-3">
            ...
            />
            <p className="flex items-center text-[11px] text-white/45 leading-snug">
              ...
              <div className="sticky top-0 z-30 grid grid-cols-[220px_1fr] items-center gap-2 border-b border-white/10 bg-[#0f0f0f] py-1 text-[11px] text-white/45">
                ...
                <div className="grid grid-cols-9">
                  {Array.from({ length: 9 }).map((_, idx) => {
                    ...
                    return (
                      <span key={hour} className="text-center font-mono">
                  ...
                  <div key={row.member.id} className="grid grid-cols-[220px_1fr] items-center gap-2">
                    ...
                    >
                      ...
                      {row.conflict ? (
                        ...
                        <div className="absolute inset-0 flex items-center justify-center text-[11px] text-white/30">Frei</div>
                      )}
                    </div>
                  </div>
                );
      ...
      {shiftEdit && (
        ...
        >
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#141414] p-5 shadow-xl">
            <h3 className="text-sm font-semibold text-white">Schicht bearbeiten</h3>
            <p className="mt-1 text-xs text-white/50">{DAY_LABELS[timelineDay]} · {shiftEdit.label}</p>
            <div className="mt-4 grid gap-3">
              <div>
                <label className="text-[10px] uppercase tracking-wide text-white/40">Start</label>
                <input
                  type="time"
                  step={900}
                  value={shiftEdit.startTime.slice(0, 5)}
                  onChange={(e) => setShiftEdit({ ...shiftEdit, startTime: e.target.value })}
                  className="mt-1 w-full rounded-lg border border-white/10 bg-[#0b0b0b] px-3 py-2 text-sm text-white"
                />
              </div>
              <div>
                <label className="text-[10px] uppercase tracking-wide text-white/40">Ende</label>
                <input
                  type="time"
                  step={900}
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              <button
                ...
                className="rounded-lg border border-[#22c55e]/35 bg-[#22c55e]/15 px-4 py-2 text-sm font-semibold text-[#86efac] hover:bg-[#22c55e]/25 disabled:opacity-50"
              >
                Speichern
              </button>
              <button
